<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Document</title>
    <link href="https://myfonts.ru/myfonts?fonts=aesthetic" rel="stylesheet" type="text/css" />
    <link rel="stylesheet" href="cssfiles/fila.css" />
    <script src="jsFiles/fila.js" defer></script>
  </head>
  <body>
    <div class="line">
      <div class="skudki">Люби ходить в спортзал</div>
    </div>
    <div class="procent">
      <div class="pryamygol"></div>
      ДО -50%
    </div>
    <div class="vce">
      <button id="esport" class="eSrortik">eSrortik</button>
      <div class="dropdown">
        <button class="dropbtn">Каталог</button>
        <div class="dropdown-content">
          <a href="#">Для зала</a>
          <a href="#">Для бега</a>
          <a href="#">Для футбола</a>
        </div>
      </div>
      <form>
        <input type="text" name="text" class="search" placeholder="Search here!" />
        <input type="submit" name="submit" class="submit" value="Search" />
      </form>
    </div>
    <div class="kartink">
      <button class="dost">
        <div class="dostaaa">Доставка</div>
        <img src="png/dostavka.png" class="img" />
      </button>
      <button id="kor" class="korz">
        <div class="tkorz">Корзина</div>
        <img src="png/korzina.png" class="pkorz" />
      </button>
      <button class="zakaz">
        <div class="tzakaz">Заказы</div>
        <img src="png/zakazi.png" class="pzakaz" />
      </button>
      <button class="profill">
        <div class="tprofill">Профиль</div>
        <img src="png/profil.png" class="pprofill" />
      </button>
    </div>
    <div class="yce">
      <button class="butskid">
        <div class="skidki">
          <div class="prforskidki"></div>
          Скидки
        </div>
      </button>
      <button id="shtanni" class="shtani">
        <div class="shtanii">Штаны</div>
      </button>

      <button id="futbol" class="futbolki">
        <div class="futbolkii">Футболки</div>
      </button>

      <button id="short" class="shotri">
        <div class="shotrii">Шорты</div>
      </button>
      <button id="koft" class="kofti">
        <div class="koftii">Кофты</div>
      </button>
      <button id="det" class="deyam">
        <div class="tdetyam">Детям</div>
      </button>
      <button id="fit" class="fitnes">
        <div class="fitness">Фитнес</div>
      </button>
      <button id="top" class="topiki">
        <div class="topikii">Топики</div>
      </button>
    </div>

    <button id="fila" class="filaa">
      <img src="png/fila.png" class="fil" />
    </button>
    <img src="adikPNG/mot.png" class="mot" />
    <div class="first">
      <button class="forMenFila" id="forMenAdik">
        <img src="filaPmg/forMen.png" class="photoForMenFila" />
      </button>
      <p class="tForMenFila">ДЛЯ МУЖЧИН</p>
      <button class="forWomenFila" id="forWomenFila">
        <img src="filaPmg/forWomen.png" class="photoForWomenFila" />
      </button>
      <p class="tForWomenFila">ДЛЯ ЖЕНЩИН</p>
      <button class="forChildrenFila" id="forChildrenFila">
        <img src="filaPmg/forChildrenFila.png" class="photoChildrenFila" />
      </button>
      <p class="tforChildrenFila">ДЛЯ ДЕТЕЙ</p>
    </div>
    <div class="second">
      <button class="odejdaFila" id="odejdaFila">
        <img src="filaPmg/odejdaFila.png" class="photoodejdaFila" />
      </button>
      <p class="tOdejdaFila">Одежда</p>
      <button class="obyvFila" id="obyvFila">
        <img src="filaPmg/obyvfila.png" class="photoObyvFila" />
      </button>
      <p class="tObyvFila">Обувь</p>
      <button class="aksFila" id="aksFila">
        <img src="filaPmg/aksFilaaa.png" class="photoAksFila" />
      </button>
      <p class="tAksFila">Аксессуры</p>
    </div>
    <div class="niz"></div>
    <button class="kompania" id="kompania">О компании</button>
  </body>
</html>
